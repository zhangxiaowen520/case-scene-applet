  ...
  <view>
    ...
    <template v-else>
      ...
      <view class="page-content" :style="{ marginTop: navBarHeight + 96 + 'px' }">
        <view v-if="currentTab !== 2" class="property-filter">
          <CustomSelect v-model="period" :options="periodOptions" placeholder="期数" @change="handlePeriodChange" />
          <CustomSelect v-model="dong" :options="dongOptions" placeholder="栋" @change="handleDongChange" />
          <CustomSelect v-model="unit" :options="unitOptions" placeholder="单元" @change="handleUnitChange" />
        </view>
...
const selectedLocation = ref(ProjectUtil.getProjectInfo().projectId);
const locations = ref([]);
...
const getUnitStructure = () => {
  requestApi.post("/room/applet/find/structure/by/project", { id: selectedLocation.value }).then((res) => {
    if (res.code === 0) {
...
const handleDongChange = (item: any) => {
  propertyList.value = [];
  const selectedPeriod = allData.value.find((period2: any) => period2.id.toString() === period.value);
  const selectedDong = selectedPeriod?.roomDongs?.find((dong: any) => dong.id.toString() === item.value);
  unitOptions.value = [
    { label: "全部", value: "" },
...
const getStatisticsData = () => {
  requestApi
    .post("/room/applet/home/statistics/number", {
      projectId: selectedLocation.value,
      unit: unit.value,
...
//获取户型
const getHouseType = () => {
  requestApi.post("/room/applet/query/house/type/by/project/id", { id: selectedLocation.value }).then((res) => {
    if (res.code === 0) {
