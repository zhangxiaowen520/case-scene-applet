...
const handleNameClick = (scope: any, index: number) => {
  ...
  uni.navigateTo({
    url: `/pages/index/effectiveCustomerChildTable?dataId=${scope.dataId}&dataName=${scope.dataName}&dataType=${scope.dataType}&queryType=${props.queryType}`
  });
};
// 选择类型
const handleTypeChange = (item: any) => {
  ...
  uni.navigateTo({
    url: `/pages/index/effectiveCustomerChildTable?dataId=${item.value}&dataName=${item.label}&dataType=${item.type}&queryType=${props.queryType}`
  });
};
//导出
const exportClick = () => {
