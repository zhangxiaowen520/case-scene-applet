...
const handleNameClick = (scope: any, index: number) => {
  ...
  uni.navigateTo({
    url: `/pages/index/effectiveVisitChildTable?dataId=${scope.dataId}&dataName=${scope.dataName}&dataType=${scope.dataType}`
  });
};
// 选择类型
const handleTypeChange = (item: any) => {
  ...
  uni.navigateTo({
    url: `/pages/index/effectiveVisitChildTable?dataId=${item.value}&dataName=${item.label}&dataType=${item.type}`
  });
};
//导出
const exportClick = () => {
  ...
  requestApi.post("/v2/home/expected/visit/export", { ...params }).then(res => {
    if (res.code === 0) {
